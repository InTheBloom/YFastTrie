        ...
        is(Unqual!(K) == long))
{
    ...
    static node_ptr merge (node_ptr lt, node_ptr rt) {
        ...
        if (rt.priority < lt.priority) {
            lt.size += rt.size;
            auto rsub = split(rt, (*lt.value)[0]);
            lt.left = merge(lt.left, rsub[0]);
            lt.right = merge(lt.right, rsub[1]);
            return lt;
        }

        rt.size += lt.size;
        auto lsub = split(lt, (*rt.value)[0]);
        rt.left = merge(rt.left, lsub[0]);
        rt.right = merge(rt.right, lsub[1]);
        return rt;
    }

    static Tuple!(node_ptr, node_ptr) split (node_ptr t, K bound) {
        if (t == null) return tuple(t, t);
        if (bound <= (*t.value)[0]) {
            auto v = split(t.left, bound);
            if (v[0] != null) t.size -= v[0].size;
            t.left = v[1];
            return tuple(v[0], t);
        }

        auto v = split(t.right, bound);
